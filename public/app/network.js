...
function hostHandle(c,m){
  ...
  if(m.type==="eastTennesseeAction"&&App.session.campaignId==="east-tennessee-1861"&&globalThis.EastTennesseeHealth){
    ...
    if(!result.ok&&result.reason==="unknown talent action")result=globalThis.EastTennesseeFinchsNest?EastTennesseeFinchsNest.performAction(App.session.campaignState,context,m.action,options):{ok:false,reason:"unknown Finch's Nest action"};
    if(!result.ok&&result.reason==="unknown Finch's Nest action")result=globalThis.EastTennesseeLickCreek?EastTennesseeLickCreek.performAction(App.session.campaignState,context,m.action,options):{ok:false,reason:"unknown Lick Creek action"};
    if(!result.ok&&result.reason==="unknown Lick Creek action")result=globalThis.EastTennesseeNPCs?EastTennesseeNPCs.performAction(App.session.campaignState,context,m.action,options):{ok:false,reason:"unknown NPC action"};
    if(!result.ok&&result.reason==="unknown NPC action")result=globalThis.EastTennesseeEquipment?EastTennesseeEquipment.performAction(App.session.campaignState,context,m.action,options):{ok:false,reason:"unknown equipment action"};
