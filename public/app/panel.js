...
function openHandout(handout){
  let modal=$("handout-modal");if(!modal){modal=document.createElement("div");modal.id="handout-modal";modal.innerHTML=`<div class="handout-shell"><div class="handout-tools"><strong id="handout-title"></strong><button id="handout-minus">−</button><button id="handout-reset">100%</button><button id="handout-plus">+</button><button id="handout-close">✕</button></div><div class="handout-viewport"><article id="handout-paper"><pre id="handout-transcript"></pre></article></div></div>`;document.body.appendChild(modal);}
  handoutZoom=1;$("handout-title").textContent=handout.title||"Handout";const paper=$("handout-paper");paper.className=`handout-paper handout-${handout.kind||"document"}`;$("handout-transcript").textContent=handout.transcript||handout.body||"";const apply=()=>{paper.style.transform=`scale(${handoutZoom})`;$("handout-reset").textContent=`${Math.round(handoutZoom*100)}%`;};$("handout-minus").onclick=()=>{handoutZoom=Math.max(.6,handoutZoom-.1);apply();};$("handout-plus").onclick=()=>{handoutZoom=Math.min(2,handoutZoom+.1);apply();};$("handout-reset").onclick=()=>{handoutZoom=1;apply();};$("handout-close").onclick=()=>modal.classList.remove("open");modal.onclick=e=>{if(e.target===modal)modal.classList.remove("open");};apply();modal.classList.add("open");
...
function etSkillHTML(actor,gm=false){
  ...
  return `<div class="sect"><h3>East Tennessee Skills · Prototype</h3>
...
function renderPanel(){
  ...
  if(App.session.campaignId==="east-tennessee-1861"){
    html+=handoutsHTML(true);
    html+=etCharactersHTML(true);
    html+=etNpcsHTML(true);
    html+=etFinchsHTML(true);
    html+=etLickCreekHTML(true);
...
function wireEtNpcWeaponChoice(p){const add=p.querySelector("#et-npc-add"),definition=p.querySelector("#et-npc-definition"),weapon=p.querySelector("#et-npc-weapon");if(!add||!definition||!weapon)return;add.onclick=()=>etHostAction({type:"gmNpcInstantiate",definitionId:definition.value,weaponChoice:weapon.value});}
...
function etLickCreekHTML(gm=false){if(App.session.campaignId!=="east-tennessee-1861"||!globalThis.EastTennesseeLickCreek)return"";const d=EastTennesseeLickCreek.DEFINITION,s=App.session.campaignState.lickCreek;if(!s?.initialized){if(!gm)return"";const inherited=App.session.campaignState.finchsNest?.lickCreekVariant||"undetermined";return`<div class="sect"><h3>Lick Creek</h3><div class="hint">Dormant scene package · Finch's Nest variant: <b>${esc(inherited)}</b>. Choose explicitly if undetermined.</div><div class="row"><select id="lc-init-variant"><option value="undetermined">choose variant</option><option value="baseline" ${inherited==="baseline"?"selected":""}>baseline</option><option value="alarmed" ${inherited==="alarmed"?"selected":""}>alarmed</option></select><select id="lc-abner"><option value="revolver">Abner: revolver</option><option value="shotgun">Abner: shotgun</option></select></div><button class="rbtn" id="lc-init">INITIALIZE LICK CREEK</button></div>`;}if(!gm)return`<div class="sect"><h3>Lick Creek</h3><div class="hint">${esc(s.scenePhase)} · ${esc(s.bridgeState)} · ${esc(s.variant)} posture</div>${(s.facts||[]).map(f=>`<details class="tok"><summary class="nm">${esc(f.label)}</summary><div class="hint">${esc(f.text)}</div></details>`).join("")}${Object.values(s.objects||{}).map(o=>`<details class="tok"><summary class="nm">${esc(o.label)}</summary><div class="hint">${esc(o.publicSummary)}</div></details>`).join("")}</div>`;
 const actorOptions=Object.values(App.session.campaignState.actors||{}).filter(a=>a.ownerKey).map(a=>`<option value="${esc(a.actorId)}">${esc(a.identity?.name||a.actorId)}</option>`).join("");return`<div class="sect"><h3>Lick Creek · Scene Control</h3><button class="rbtn" id="lc-map">SHOW TACTICAL MAP</button><label class="check"><input id="lc-overlay" type="checkbox" ${s.overlayVisible?"checked":""}> show advisory GM overlay cards</label><select id="lc-phase" style="width:100%">${[...EastTennesseeLickCreek.PHASES].map(x=>`<option ${x===s.scenePhase?"selected":""}>${x}</option>`).join("")}</select><div class="tok"><span class="nm">${esc(s.variant.toUpperCase())} · ${Object.keys(s.npcIds).filter(x=>x==="vance"||x==="rader"||x==="mose"||x.startsWith("guard-")).length} guards</span><div class="hint">Bridge: <b>${esc(s.bridgeState)}</b> · phase: ${esc(s.scenePhase)}</div></div>${s.overlayVisible?d.gmOverlay.map(x=>`<div class="hint"><b>${esc(x.id)}:</b> ${esc(x.note)}</div>`).join(""):""}
