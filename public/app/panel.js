...
function renderEditorPanel(){
  ...
  $("lv-name").onchange=e=>{edSnapshot();App.document.level.name=e.target.value.trim()||"Untitled Level";$("tab-verso").textContent=App.session.campaignId==="east-tennessee-1861"?"CAMPAIGN TABLE":App.document.level.name.toUpperCase();levelTouched();};
...
function etSceneApplyPresentation(level,pkg){return globalThis.EastTennesseeScenes?.applyPresentation?.(level,pkg)||etSceneClone(level);}
...
function openEtSceneLevel(key,{announce=true}={}){
  ...
  let level=state.sceneLevels[key]?etSceneApplyPresentation(state.sceneLevels[key],pkg):etSceneClone(pkg.level);
...
function wireEtFinchs(p,gm=false){if(!gm||!globalThis.EastTennesseeFinchsNest)return;const init=p.querySelector("#fn-init");if(init)init.onclick=()=>{etHostAction({type:"gmFinchsInitialize"});showFinchsFloor(App.session.campaignState.finchsNest.activeFloor);};p.querySelectorAll("[data-fn-floor]").forEach(el=>el.onclick=()=>{etHostAction({type:"gmFinchsSetFloor",floor:el.dataset.fnFloor});showFinchsFloor(el.dataset.fnFloor);});const phase=p.querySelector("#fn-phase");if(phase)phase.onchange=()=>{etHostAction({type:"gmFinchsSetPhase",phaseId:phase.value});showFinchsFloor(App.session.campaignState.finchsNest.activeFloor);};const overlay=p.querySelector("#fn-overlay");if(overlay)overlay.onchange=()=>etHostAction({type:"gmFinchsSetOverlay",visible:overlay.checked});p.querySelectorAll("[data-fn-region]").forEach(el=>el.onclick=()=>etHostAction({type:"gmFinchsReveal",domain:"region",id:el.dataset.fnRegion,revealed:!App.session.campaignState.finchsNest.revealedRegions.includes(el.dataset.fnRegion)}));p.querySelectorAll("[data-fn-object]").forEach(el=>el.onclick=()=>etHostAction({type:"gmFinchsReveal",domain:"object",id:el.dataset.fnObject,revealed:!App.session.campaignState.finchsNest.revealedObjects.includes(el.dataset.fnObject)}));const caseSave=p.querySelector("#fn-case-save");if(caseSave)caseSave.onclick=()=>{const state={location:p.querySelector("#fn-case-location").value,sealCondition:p.querySelector("#fn-case-seal").value,gmNotes:p.querySelector("#fn-case-notes").value};p.querySelectorAll("[data-fn-case]").forEach(el=>state[el.dataset.fnCase]=el.checked);etHostAction({type:"gmFinchsSetCase",caseState:state});};const witness=p.querySelector("#fn-witness-save");if(witness)witness.onclick=()=>{const notes={};p.querySelectorAll("[data-fn-witness]").forEach(el=>notes[el.dataset.fnWitness]=el.value);etHostAction({type:"gmFinchsSetWitnesses",witnessNotes:notes,evidenceNotes:p.querySelector("#fn-evidence").value});};const outcome=p.querySelector("#fn-outcome-save");if(outcome)outcome.onclick=()=>etHostAction({type:"gmFinchsSetOutcome",courierOutcome:p.querySelector("#fn-outcome").value,revealed:p.querySelector("#fn-outcome-reveal").checked});const variant=p.querySelector("#fn-variant-save");if(variant)variant.onclick=()=>etHostAction({type:"gmFinchsConfirmVariant",variant:p.querySelector("#fn-variant").value});const reset=p.querySelector("#fn-reset");if(reset)reset.onclick=()=>{if(confirm("Reset local Finch's Nest phase, placements, case, and reveals? Committed witnesses, evidence, outcome, and Lick Creek variant will be preserved.")){etHostAction({type:"gmFinchsReset",confirm:true});showFinchsFloor("ground");}};}
 ...
 const ET_LICK_PHASE_INFO=Object.freeze({
  reconnaissance:["Reconnaissance","scouting, waiting, ordinary observation"],
  approach:["Approach","choosing or beginning a route"],
  infiltration:["Infiltration","inside the actionable space, before alarm"],
  alarm:["Alarm","immediate opposing action has begun"],
  sabotage:["Sabotage","working on the bridge objective"],
  fire:["Fire","a fire process is active"],
  escape:["Escape","leaving under an active threat"],
  resolved:["Resolved","immediate scene is over"],
  withdrawn:["Withdrawn","unseen withdrawal; bridge remains intact"]
 });
 function etLickPhaseLabel(phase){return ET_LICK_PHASE_INFO[phase]?.[0]||String(phase||"").replace(/([A-Z])/g," $1").replace(/^./,x=>x.toUpperCase());}
 function etLickPhaseOptions(selected){return [...EastTennesseeLickCreek.PHASES].map(phase=>{const info=ET_LICK_PHASE_INFO[phase];return`<option value="${phase}" ${phase===selected?"selected":""}>${esc(info?`${info[0]} · ${info[1]}`:etLickPhaseLabel(phase))}</option>`;}).join("");}
 function etLickBridgeLabel(state){return String(state||"").replace(/([A-Z])/g," $1").replace(/^./,x=>x.toUpperCase());}
function etLickCreekHTML(gm=false){if(App.session.campaignId!=="east-tennessee-1861"||!globalThis.EastTennesseeLickCreek)return"";const d=EastTennesseeLickCreek.DEFINITION,s=App.session.campaignState.lickCreek;if(!s?.initialized){if(!gm)return"";const inherited=App.session.campaignState.finchsNest?.lickCreekVariant||"undetermined";return`<div class="sect"><h3>Lick Creek</h3><div class="hint">Dormant scene package · Finch's Nest variant: <b>${esc(inherited)}</b>.<br>${esc(etLickVariantNote(inherited))}</div><div class="row"><select id="lc-init-variant"><option value="undetermined">choose variant</option><option value="baseline" ${inherited==="baseline"?"selected":""}>baseline · 6 guards · routine</option><option value="alarmed" ${inherited==="alarmed"?"selected":""}>alarmed · 8 guards · alert</option></select></div><button class="rbtn" id="lc-init">INITIALIZE &amp; OPEN ISOMETRIC SCENE</button></div>`;}if(!gm)return`<div class="sect"><h3>Lick Creek</h3><div class="hint">${esc(s.scenePhase)} · ${esc(s.bridgeState)} · ${esc(s.variant)} posture</div>${(s.facts||[]).map(f=>`<details class="tok"><summary class="nm">${esc(f.label)}</summary><div class="hint">${esc(f.text)}</div></details>`).join("")}${Object.values(s.objects||{}).map(o=>`<details class="tok"><summary class="nm">${esc(o.label)}</summary><div class="hint">${esc(o.publicSummary)}</div></details>`).join("")}</div>`;
 const actorOptions=Object.values(App.session.campaignState.actors||{}).filter(a=>a.ownerKey).map(a=>`<option value="${esc(a.actorId)}">${esc(a.identity?.name||a.actorId)}</option>`).join("");const phaseNote=EastTennesseeLickCreek.PHASE_GUIDANCE?.[s.scenePhase]||"This is a GM bookmark; it does not automate the scene.";return`<div class="sect"><h3>Lick Creek · Scene Control</h3><button class="rbtn" id="lc-map">OPEN ISOMETRIC SCENE</button><label class="check"><input id="lc-overlay" type="checkbox" ${s.overlayVisible?"checked":""}> show advisory GM overlay cards</label><label class="et-control-label" for="lc-phase">Adventure phase</label><select id="lc-phase" style="width:100%">${etLickPhaseOptions(s.scenePhase)}</select><div class="hint"><b>Tracker only.</b> ${esc(phaseNote)}</div><div class="tok"><div class="hint"><b>Variant: ${esc(s.variant.toUpperCase())} · ${Object.keys(s.npcIds).filter(x=>x==="vance"||x==="rader"||x==="mose"||x.startsWith("guard-")).length} guards</b><br>${esc(etLickVariantNote(s.variant))}<br>Bridge: <b>${esc(etLickBridgeLabel(s.bridgeState))}</b> · phase: ${esc(etLickPhaseLabel(s.scenePhase))}</div></div>${s.overlayVisible?etOverlayNotesHTML(d.gmOverlay):""}
 ...
 <details><summary>Bridge and fire</summary><label class="et-control-label" for="lc-bridge">Bridge condition</label><div class="row"><select id="lc-bridge">${[...EastTennesseeLickCreek.BRIDGE_STATES].map(x=>`<option value="${x}" ${x===s.bridgeState?"selected":""}>${esc(etLickBridgeLabel(x))}</option>`).join("")}</select><button class="rbtn quiet" id="lc-bridge-save">SET BRIDGE</button></div><div class="hint">A GM record of the bridge’s fictional condition. It does not apply geometry, line of sight, or automatic sabotage results.</div><label class="et-control-label" for="lc-fire-kind">Fire procedure</label><div class="row"><select id="lc-fire-kind"><option value="weak">Weak · d6</option><option value="competent">Competent · d4</option><option value="exceptional">Exceptional · d3</option><option value="custom">Custom rounds</option></select><input id="lc-fire-rounds" type="number" min="1" max="99" placeholder="override"><button class="rbtn" id="lc-fire-start" ${["stageOne","stageTwo"].includes(s.fireState.stage)?"disabled":""}>START STAGE ONE</button></div><div class="hint">This chooses the starting countdown procedure only. The timer advances with structured rounds after the GM starts it.</div><div class="row"><input id="lc-stage2-rounds" type="number" min="1" max="99" placeholder="d4"><button class="rbtn quiet" id="lc-stage2" ${(!App.session.campaignState.adventureFlags?.bridgeDisabled&&s.bridgeState!=="disabled")||s.fireState.stageTwoTimerId?"disabled":""}>START STAGE TWO</button><button class="rbtn quiet" id="lc-extinguish">EXTINGUISH</button></div><div class="hint">Stage two begins automatically when stage one disables the bridge. Pause, resume, adjust, remove, or resolve fire timers in Structured Play.</div></details>
