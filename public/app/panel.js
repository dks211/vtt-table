...
function renderClientPanel(){
  ...
  if(App.session.campaignId==="east-tennessee-1861"&&!mineActor)html+=etCharactersHTML(false)+etNpcsHTML(false)+etFinchsHTML(false)+etLickCreekHTML(false);
  if(mineActor){
    html+=etCharactersHTML(false)+etNpcsHTML(false)+etFinchsHTML(false)+etLickCreekHTML(false)+etTalentControlsHTML(mineActor,false)+etHealthHTML(mineActor,false)+etSkillHTML(mineActor,false)+etEquipmentHTML(mineActor,false)+etCombatHTML(mineActor,false);
