...
function etLickCreekHTML(gm=false){if(App.session.campaignId!=="east-tennessee-1861"||!globalThis.EastTennesseeLickCreek)return"";const d=EastTennesseeLickCreek.DEFINITION,s=App.session.campaignState.lickCreek;if(!s?.initialized){if(!gm)return"";const inherited=App.session.campaignState.finchsNest?.lickCreekVariant||"undetermined";return`<div class="sect"><h3>Lick Creek</h3><div class="hint">Dormant scene package · Finch's Nest variant: <b>${esc(inherited)}</b>.<br>${esc(etLickVariantNote(inherited))}</div><div class="row"><select id="lc-init-variant"><option value="undetermined">choose variant</option><option value="baseline" ${inherited==="baseline"?"selected":""}>baseline · 6 guards · routine</option><option value="alarmed" ${inherited==="alarmed"?"selected":""}>alarmed · 8 guards · alert</option></select><span class="hint">Abner Raines carries a revolver.</span></div><button class="rbtn" id="lc-init">INITIALIZE &amp; OPEN ISOMETRIC SCENE</button></div>`;}if(!gm)return`<div class="sect"><h3>Lick Creek</h3><div class="hint">${esc(s.scenePhase)} · ${esc(s.bridgeState)} · ${esc(s.variant)} posture</div>${(s.facts||[]).map(f=>`<details class="tok"><summary class="nm">${esc(f.label)}</summary><div class="hint">${esc(f.text)}</div></details>`).join("")}${Object.values(s.objects||{}).map(o=>`<details class="tok"><summary class="nm">${esc(o.label)}</summary><div class="hint">${esc(o.publicSummary)}</div></details>`).join("")}</div>`;
 const actorOptions=Object.values(App.session.campaignState.actors||{}).filter(a=>a.ownerKey).map(a=>`<option value="${esc(a.actorId)}">${esc(a.identity?.name||a.actorId)}</option>`).join("");return`<div class="sect"><h3>Lick Creek · Scene Control</h3><button class="rbtn" id="lc-map">OPEN ISOMETRIC SCENE</button><label class="check"><input id="lc-overlay" type="checkbox" ${s.overlayVisible?"checked":""}> show advisory GM overlay cards</label><select id="lc-phase" style="width:100%">${[...EastTennesseeLickCreek.PHASES].map(x=>`<option ${x===s.scenePhase?"selected":""}>${x}</option>`).join("")}</select><div class="tok"><div class="hint"><b>Variant: ${esc(s.variant.toUpperCase())} · ${Object.keys(s.npcIds).filter(x=>x==="vance"||x==="rader"||x==="mose"||x.startsWith("guard-")).length} guards</b><br>${esc(etLickVariantNote(s.variant))}<br>Bridge: <b>${esc(s.bridgeState)}</b> · phase: ${esc(s.scenePhase)}</div></div>${s.overlayVisible?etOverlayNotesHTML(d.gmOverlay):""}
