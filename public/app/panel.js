...
function etCombatHTML(actor,gm=false){
  if(!actor||!globalThis.EastTennesseeCombat)return"";const state=App.session.campaignState,pending=state.pendingAttack,last=(state.attacks||[]).slice(-1)[0],weapons=(actor.weaponIds||[]).map(id=>EastTennesseeCombat.WEAPONS[id]).filter(Boolean),targets=Object.values(state.actors||{}).filter(a=>a.actorId!==actor.actorId&&!a.health?.dead);
...
function wireEtCombat(p,actor,gm=false){if(!actor||!globalThis.EastTennesseeCombat)return;const state=App.session.campaignState,sp=state.structuredPlay||{},send=action=>gm?etHostAction(action):clientSend({type:"eastTennesseeAction",action:{...action,expectedRound:sp.roundNumber,expectedEntryId:sp.currentEntryId}}),weapon=()=>$("et-weapon")?.value,target=()=>$("et-attack-target")?.value;
  const aim=$("et-aim");if(aim)aim.onclick=()=>send({type:"declareAim",actorId:actor.actorId,weaponId:weapon(),targetActorId:target(),description:"Maintaining firing position"});const cancel=$("et-cancel-aim");if(cancel)cancel.onclick=()=>send({type:"cancelAim",actorId:actor.actorId});const attack=$("et-attack");if(attack)attack.onclick=()=>{const ready=(sp.initiativeEntries||[]).find(e=>e.triggeredReady&&(e.participantIds||[]).some(id=>(sp.participants||[]).find(x=>x.id===id)?.actorId===actor.actorId));send({type:"requestAttack",attackerActorId:actor.actorId,targetActorId:target(),weaponId:weapon(),calledShot:$("et-called").value||null,calledShotDescription:$("et-called-desc").value,nonlethal:$("et-nonlethal").checked,readyEntryId:ready?.entryId});};
...
function renderPanel(){
  ...
  if(etSelected)html+=etHealthHTML(etSelected,true)+etSkillHTML(etSelected,true)+etCombatHTML(etSelected,true);
  ...
  const panelGroup=section=>{
    ...
    if(title==="Tokens"||title.startsWith("Roll as ")||title.startsWith("Sheet ·")||title==="Add Token"||title.startsWith("East Tennessee Health")||title.startsWith("East Tennessee Skills")||title.startsWith("East Tennessee Attacks"))return"tokens";
    return"scene";
  };
...
function renderClientPanel(){
  ...
  if(mineActor){
    html+=etHealthHTML(mineActor,false)+etSkillHTML(mineActor,false)+etCombatHTML(mineActor,false);
