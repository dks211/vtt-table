...
function etNpcsHTML(gm=false){if(App.session.campaignId!=="east-tennessee-1861"||!globalThis.EastTennesseeNPCs)return"";const s=App.session.campaignState,defs=EastTennesseeNPCs.DEFINITIONS,npcs=Object.values(s.npcs||{});if(!gm)return`<div class="sect"><h3>People in the Scene</h3>${npcs.filter(n=>n.active&&n.sceneStatus!=="absent").map(n=>{const a=s.actors[n.actorId],badge=EastTennesseeNPCs.tokenBadge(s,n.id);return`<details class="tok"><summary class="nm">${esc(a?.identity?.name||n.visibleLabel)}</summary><div class="hint">${esc(a?.public?.role||"")} · ${esc(a?.health?.state||"unhurt")}${badge?` · <b>${esc(badge)}</b>`:""}${n.moraleRevealed?` · ${esc(n.moraleState)}`:""}<br>${esc(n.publicNotes||a?.public?.summary||"")}</div></details>`;}).join("")||`<div class="hint">No revealed people are active.</div>`}</div>`;
 const options=Object.values(defs).map(d=>`<option value="${d.id}">${esc(d.displayName)} · ${esc(d.npcClass)}</option>`).join("");return`<div class="sect"><h3>East Tennessee NPC Roster</h3><div class="row"><select id="et-npc-definition" style="flex:1">${options}</select><button class="rbtn" id="et-npc-add">ADD</button></div><div class="hint">Definitions are immutable; live health, ammunition, status, and notes persist separately.</div>${npcs.map(n=>{const d=defs[n.definitionId],a=s.actors[n.actorId],weapons=(a.weaponInstanceIds||[]).map(id=>s.weapons?.[id]).filter(Boolean),badge=EastTennesseeNPCs.tokenBadge(s,n.id),skills=Object.entries(d.skills).map(([k,v])=>`${k} ${v}`).join(" · ")||"No defined checks";return`<details class="tok"><summary class="nm">${esc(a.identity?.name||n.visibleLabel)} ${badge?`· ${esc(badge)}`:""}</summary><div class="hint"><b>${esc(d.role)}</b> · ${esc(n.npcClass)}<br>${esc(a.health.state)} · ${weapons.map(w=>{const wd=EastTennesseeCombat.WEAPONS[w.definitionId];return`${wd.name}${wd.firearmType?` ${EastTennesseeEquipment.loaded(s,w)}/${wd.capacity}`:""}`;}).join(" · ")||"no visible weapon"}<br>${esc(skills)}<br><b>Guidance:</b> ${esc(d.moraleGuidance)}</div><div class="row"><select data-npc-morale="${n.id}">${[...EastTennesseeNPCs.MORALE].map(x=>`<option ${n.moraleState===x?"selected":""}>${x}</option>`)}</select><select data-npc-disposition="${n.id}">${[...EastTennesseeNPCs.DISPOSITIONS].map(x=>`<option ${n.disposition===x?"selected":""}>${x}</option>`)}</select><select data-npc-status="${n.id}">${[...EastTennesseeNPCs.STATUSES].map(x=>`<option ${n.sceneStatus===x?"selected":""}>${x}</option>`)}</select></div><div class="row"><select data-npc-class="${n.id}">${[...EastTennesseeNPCs.CLASSES].map(x=>`<option ${n.npcClass===x?"selected":""}>${x}</option>`)}</select><button class="rbtn quiet" data-npc-reveal="${n.id}">${n.identityRevealed?"CONCEAL":"REVEAL"}</button><button class="rbtn quiet" data-npc-restore="${n.id}">RESTORE</button></div><div class="row"><input data-npc-label="${n.id}" value="${esc(n.visibleLabel)}" placeholder="visible label" style="flex:1"><input data-npc-delegate="${n.id}" value="${esc(n.delegatedOwnerKey||"")}" placeholder="temporary player key" style="flex:1"></div><textarea data-npc-notes="${n.id}" placeholder="GM notes">${esc(n.gmNotes||"")}</textarea><button class="rbtn quiet" data-npc-save="${n.id}">SAVE LABEL / DELEGATION / NOTES</button></details>`;}).join("")}</div>`;}
