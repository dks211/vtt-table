...
function etLickCreekHTML(gm=false){if(App.session.campaignId!=="east-tennessee-1861"||!globalThis.EastTennesseeLickCreek)return"";const d=EastTennesseeLickCreek.DEFINITION,s=App.session.campaignState.lickCreek;if(!s?.initialized){if(!gm)return"";const inherited=App.session.campaignState.finchsNest?.lickCreekVariant||"undetermined";return`<div class="sect"><h3>Lick Creek</h3><div class="hint">Dormant scene package · Finch's Nest variant: <b>${esc(inherited)}</b>.<br>${esc(etLickVariantNote(inherited))}</div><div class="row"><select id="lc-init-variant"><option value="undetermined">choose variant</option><option value="baseline" ${inherited==="baseline"?"selected":""}>baseline · 6 guards · routine</option><option value="alarmed" ${inherited==="alarmed"?"selected":""}>alarmed · 8 guards · alert</option></select><span class="hint">Abner Raines carries a revolver.</span></div><button class="rbtn" id="lc-init">INITIALIZE &amp; OPEN MAP</button></div>`;}if(!gm)return`<div class="sect"><h3>Lick Creek</h3><div class="hint">${esc(s.scenePhase)} · ${esc(s.bridgeState)} · ${esc(s.variant)} posture</div>${(s.facts||[]).map(f=>`<details class="tok"><summary class="nm">${esc(f.label)}</summary><div class="hint">${esc(f.text)}</div></details>`).join("")}${Object.values(s.objects||{}).map(o=>`<details class="tok"><summary class="nm">${esc(o.label)}</summary><div class="hint">${esc(o.publicSummary)}</div></details>`).join("")}</div>`;
