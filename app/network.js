...
function hostHandle(c,m){
  ...
  if(m.type==="eastTennesseeAction"&&App.session.campaignId==="east-tennessee-1861"&&globalThis.EastTennesseeHealth){
    ...
    let result=globalThis.EastTennesseeRounds?EastTennesseeRounds.performAction(App.session.campaignState,context,m.action,options):{ok:false,reason:"unknown structured-play action"};
